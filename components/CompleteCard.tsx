...
const CompleteCard = ({ name, index, className }: CompleteCardProps) => {
  ...
  useEffect(() => {
    ...
    logoImage.src = '/assets/images/court-attack-logo.svg';
    
    // Background image 로드
    const bgImage = new Image();
    bgImage.src = '/assets/images/complete-card-bg.svg';
    ...
    Promise.all([
      ...
      // name 부분 (bold)
      ctx.font = 'bold 40px SUIT';
      ctx.fillText(name, 36, 130);
      
      // '님이' 부분 (medium)
      ctx.font = '500 40px SUIT';
      ...
      // Index number 렌더링
      ctx.font = '900 100px SUIT';
      ctx.fillText(`${index.toLocaleString()}번째`, 36, 240);

      // Description text 렌더링
      ctx.font = '500 30px SUIT';
      if (index <= 500) {
      ...
      ctx.drawImage(bgImage, 0, BASE_SIZE - 152, BASE_SIZE, 152);
    });
  }, [name, index]);
